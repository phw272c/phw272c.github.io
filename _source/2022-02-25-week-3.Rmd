...
It's possible to use different bandwidths. The larger the bandwidth, the smoother the density estimate. 
```{r}
...
But this is just a density of cases, e.g. it doesn't account for the denominator - the controls. To do this, we can use the Kelsall & Diggle method (see paper in course notes), which calculates the ratio of the density estimate of cases:controls
...
We can plot a summary plot using ther Lowes parameter. The lowess option gives us LOWESS curves for the relationship between x and y.  LOWESS stands for locally weighted scatterplot smoothing.
```{r}
